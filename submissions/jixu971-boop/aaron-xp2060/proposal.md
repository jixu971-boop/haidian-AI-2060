...
本次补入独立、可编辑的 XP 标识资产：绿色 #2F7D55 表示自然预算的先行约束，暖色 #D17842 表示自然人判断、申诉和批准，蓝色 #2967B1 表示机器版本与可回滚接口；深墨 #172033 只承担文字和轮廓。标识保留至少一个端点直径的留白，不拉伸、不闭合三条线、不以单色替代三种责任、也不与任何机构标志并置造成背书含义。最小应用单元为“状态—责任人—日期—来源—AI/版权提示—AI-off 与无障碍路径”，可用于 XP·OPEN、XP·TEST、XP·FORUM、XP·FAILSAFE 四类活动子标，而不宣称活动已获批准或已由任何主体主办。[source:AGENT-TASKBOOK] [source:CULTURE-PUBLISHER-ARCHIVE]

![Aaron-XP2060 开放 XP 标识：自然、自然人、机器三线与七道决策门](assets/figures/identity-decision-system-20260902.png)
...
为避免把主体、许可或预算的不确定性伪装成已经落实，本方案将每一个 P01—P09 / E01—E04 转成同一份“决策包”。G0 确认责任与非报复申诉，G1 确认场地和权利，G2 明确专业审查与许可路径，G3 批准资源与退场预算包络，G4 核验数据、版权和安全，G5 演示人工等价服务和限期试运行，G6 由独立复盘决定扩大、修订、退役或恢复原状。七道门的计数见 [metric:decision_gate_count]，13 个项目/活动的映射见 [metric:implementation_decision_record_count]；它们是决策条件清单，不是任何审批、资金或合作承诺。[depth:renewal_project_list] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]

每一门都要求留下可复核材料而非口头判断：责任人和申诉路由、场地/恢复记录、适用的专业审查清单、最低资源与成本类别、版本与权利材料、AI-off 演示与停机演练、独立复盘与退出决定。若任一材料缺失，默认结论是“不扩大”；若触发 S0，则立即停用功能、转交人工、保留证据并进入 G6。完整项目映射与决策包字段已写入本方案及任务覆盖矩阵。[metric:implementation_operation_item_count] [depth:renewal_project_list]
